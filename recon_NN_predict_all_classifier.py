...
def main():
    ...
    for qi_name in qis:
        ...
        for deid_filename in sdg_practice_problems:
            ...
            recon = nn_all_at_once_reconstruction(deid, targets, qi, hidden_features)
...
def nn_all_at_once_reconstruction(deid, targets, qi, hidden_features, test_size=test_size_, hidden_dims=hidden_dims_, batch_size=batch_size_, learning_rate=learning_rate_, epochs=epochs_):
    ...
    targets_dataset = CategoricalDataset(targets_encoded, np.zeros((len(targets_encoded), y_encoded.shape[1])))
    ...
    num_logits = y_encoded.shape[1]

    # Create model
    model = CategoricalNN(input_dim, hidden_dims, num_logits)
    ...
    # Train model
    trained_model, history = train_model(model, train_loader, test_loader, criterion, optimizer, device, y_encoder, epochs)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device, y_encoder, epochs=50, early_stopping_patience=patience):
    model.to(device)
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            ...
            loss, _ = custom_criterion_for_multiple_class_logits(outputs, labels, y_encoder)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                ...
                loss, accuracy = custom_criterion_for_multiple_class_logits(outputs, labels, y_encoder)
                val_accuracy += accuracy.item() / len(y_encoder.categories_) * inputs.size(0)
...
def custom_criterion_for_multiple_class_logits(outputs, labels, y_encoder):
    feature_output_indeces_start = 0
    loss = torch.tensor(0.)
    accuracy = torch.tensor(0.)
    for feature_classes in y_encoder.categories_:
        one_feature_outputs = outputs[:, feature_output_indeces_start: feature_output_indeces_start + len(feature_classes)]
        one_feature_targets = labels[:, feature_output_indeces_start: feature_output_indeces_start + len(feature_classes)]
        one_feature_target_indices = torch.argmax(one_feature_targets, dim=1)
        one_feature_output_indices = torch.argmax(one_feature_outputs, dim=1)
        # loss += criterion(one_feature_outputs, one_feature_target_indices)
        loss += nn.functional.cross_entropy(one_feature_outputs, one_feature_target_indices)
        accuracy += (one_feature_output_indices == one_feature_target_indices).sum() / len(one_feature_target_indices)
        feature_output_indeces_start += len(feature_classes)
    return loss, accuracy

# Function to make predictions
def predict(model, targets_loader, device, y_encoder):
    model.eval()
    OH_predictions_all = []
    true_labels = []

    with torch.no_grad():
        for inputs, labels in targets_loader:
            inputs = inputs.to(device)
            outputs = model(inputs)
            OH_predictions = np.empty((len(inputs), 0))
            feature_output_indeces_start = 0
            for feature_classes in y_encoder.categories_:
                one_feature_outputs = outputs[:,feature_output_indeces_start: feature_output_indeces_start + len(feature_classes)]
                pred_indeces = torch.argmax(one_feature_outputs, dim=1)
                OH_predictions = np.concatenate((OH_predictions, np.eye(len(feature_classes))[pred_indeces]), axis=1)
                feature_output_indeces_start += len(feature_classes)

            OH_predictions_all.extend(OH_predictions)

    return np.array(OH_predictions_all)


# Process categorical features
def process_categorical_data(df, qi, hidden_columns, targets):
    # Separate features and target
    X = df[qi].values
    y = df[hidden_columns].values

    # Set up one-hot encoding for categorical features
    encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
    X_encoded = encoder.fit_transform(X)
    targets_encoded = encoder.transform(targets[qi])

    # Encode target if it's categorical
    y_encoder = OneHotEncoder(sparse_output=False, handle_unknown='ignore')
    y_encoded = y_encoder.fit_transform(y)

    return X_encoded, y_encoded, targets_encoded, encoder, y_encoder
