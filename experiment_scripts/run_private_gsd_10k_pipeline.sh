...
timeout 6h python experiment_scripts/generate_new_dp_sweep.py \
  ...
  --methods PrivateGSD \
  --samples 0 \
  --workers 2 || echo "  (generation step exited non-zero / timed out -- continuing to attack step with whatever synth.csv files exist)"
