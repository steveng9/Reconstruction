...
DATASET_CONFIGS: dict[str, dict] = {
    "adult": {
        ...
        "n_features":  None,
        "qi_variants": ["QI_linear"],   # Table 7's QI: hides exactly `income`
    },
    "cdc_diabetes": {
        "base":        "cdc_diabetes",
        "name":        "cdc_diabetes",
...
# ── SDG methods ───────────────────────────────────────────────────────────────
SDG_METHODS = [
    # ── The four new DP generators, at the two budgets Table 7 shows.
    ("PrivBayes",  {"epsilon": 1.0}),
    ("PrivBayes",  {"epsilon": 1000.0}),
    ("PrivSyn",    {"epsilon": 1.0}),
    ("PrivSyn",    {"epsilon": 1000.0}),
    ("MWEMPGM",    {"epsilon": 1.0}),
    ("MWEMPGM",    {"epsilon": 1000.0}),
    ("PrivateGSD", {"epsilon": 1.0}),
    ("PrivateGSD", {"epsilon": 1000.0}),
    # ── Every EXISTING Table 7 row, re-run in the same batch.
    # Rationale: the first pass reproduced CellSuppression to 0.1 pp but moved MST
    # eps=1 by 0.8 pp, even though that synth file is untouched since Feb 15 — i.e.
    # the residual is attack stochasticity (RF/MLP seeds), not data. Mixing a fresh
    # measurement of the new rows with the older measurement of the old rows would
    # bake ~0.5 pp of jitter into a table whose whole point is a 0.3 pp vs 20 pp
    # contrast. One batch, one measurement.
    ("CellSuppression", {}),
    ("RankSwap",        {}),
    ("TabDDPM",         {}),
    ("Synthpop",        {}),
...
ATTACK_CONFIGS = [
    # Exactly the three attacks Table 7 averages.
    ("RandomForest", {}, ""),
    ("KNN",          {}, ""),
    ("MLP",          {}, ""),
]
